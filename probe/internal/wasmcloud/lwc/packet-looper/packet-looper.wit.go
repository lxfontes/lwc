...
// Package packetlooper represents the exported interface "wasmcloud:lwc/packet-looper".
...
//go:wasmexport wasmcloud:lwc/packet-looper#process
//export wasmcloud:lwc/packet-looper#process
func wasmexport_Process(packet0 uint64, ttl0 uint64, chaos0 uint32, route0 uint32, route1 *uint8, route2 uint32) {
	packet := (uint64)((uint64)(packet0))
	ttl := (uint64)((uint64)(ttl0))
	chaos := (uint16)((uint32)(chaos0))
	route := lift_OptionString((uint32)(route0), (*uint8)(route1), (uint32)(route2))
	Exports.Process(packet, ttl, chaos, route)
	return
}
